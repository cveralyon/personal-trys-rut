<template>
    <div v-if="$auth.loggedIn" class="maincont">
        <div class="bg-gray-100 rounded-md cont2">
            <h2 class="bienvenida"><span style="color:#33aca6;">¡Hola amigos de {{ $auth.user.company_name }}!</span> ✌  Aquí se visualizan las reservas pasadas:</h2>
    ...
    methods: {
        async getReservations() {
            if (!this.$auth.loggedIn) {
                ...
                const { data } = await this.$axios.get("/api/v1/reservations/company_reserves/"+ this.$auth.user.company_id);
                console.log(data)
